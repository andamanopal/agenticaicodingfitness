#!/usr/bin/env python3
"""Shared configuration for the Week 23 **Stack Navigator** hub app.

The navigator runs in one of two modes, auto-detected at import:
  ...
  • SIM  — no endpoint is reachable. Each stack node falls back to a faithful
    SIMULATOR (sim.py): the node's answer streams token-by-token at plausible
    DGX-Spark tok/s. The exact commands you'd run on real hardware are always
    shown either way, so SIM is a dry-run of the real thing.
